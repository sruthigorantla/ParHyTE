...
int relation_num,entity_num,time_num,feature_num,nyt_relation_num;
...
public:
	...
	//std::cout << "fb_t size= " << fb_t.size() << endl;
...
///////////////////////////////////////////////////////////////////////////////////////////// change from here
        ...
        {   
            ...
            for (int eval=0; eval<nepoch/nthread; eval++)
            {
			    ...
			    //std::cout << fb_h[i] << " " << fb_r[i] << " " << fb_l[i] << " " << fb_t[i] << endl;
                            train_kb(fb_h[i],fb_l[i],fb_r[i],fb_t[i],fb_h[i],j,fb_r[i],fb_t[i],private_res);
                        }
                        else
                        {
                            while(true)
			    ...
			    //std::cout << fb_h[i] << " " << fb_r[i] << " " << fb_l[i] << " " << fb_t[i] << endl;
                            train_kb(fb_h[i],fb_l[i],fb_r[i],fb_t[i],j,fb_l[i],fb_r[i],fb_t[i],private_res);
                        }
                ...
                stop = time(NULL);
		/*
		printf("hereeeeeeeeeeeeee %d ",entity_num);	
		for(int ind = 0; ind < entity_num ; ind++){
			private_res += vec_len(entity_vec[ind]);
            	}
		*/
		printf("thread id:%d,epoch: %d,res: %f, time:%d\n", omp_get_thread_num(),eval,private_res,int(stop-start));
	    }
    ...
    double res1;
    double calc_sum(int e1,int e2,int rel,int tim)
    {
	
        double tmp1=0,tmp2=0,tmp3=0;
	/*
	std::cout << e1 << " " << e2 << " " << rel << " " << tim << endl;
	std::cout << A[tim].size() << endl << entity_vec[e1].size() << endl;
	std::cout << entity_vec[e2].size() << endl;
	std::cout << relation_vec[rel].size() << endl;        
	*/
	for (int jj=0; jj<n; jj++)
        {
    ...
    void gradient(int e1,int e2,int rel,int tim,double belta)
    {
        double tmp1 = 0, tmp2 = 0, tmp3 = 0 ;
        double sum_x=0;
        for (int jj=0; jj<n; jj++)
	...
	int len1 = vec_len(entity_vec[e1]);
	int len2 = vec_len(entity_vec[e2]);
        for (int ii=0; ii<n; ii++)
        {
            
            double x = 2*(entity_vec[e2][ii]-tmp2*A[tim][ii]-(entity_vec[e1][ii]-tmp1*A[tim][ii])-(relation_vec[rel][ii]-tmp3*A[tim][ii]));
            ...
            //entity_vec[e1][ii]-=belta*rate*(x+(entity_vec[e1][ii]/len1));
            //entity_vec[e2][ii]+=belta*rate*(x+(entity_vec[e2][ii]/len2));
            entity_vec[e1][ii] -= belta*rate*x;
	    entity_vec[e2][ii] += belta*rate*x;
	    A[tim][ii]+=belta*rate*x*tmp1;
            A[tim][ii]-=belta*rate*x*tmp2;
	    A[tim][ii]+=belta*rate*x*tmp3;
        ...
        for (int ii=0; ii<n; ii++)
        {
            ...
            A[tim][ii]+=belta*rate*sum_x*entity_vec[e1][ii];
            A[tim][ii]-=belta*rate*sum_x*entity_vec[e2][ii];
            A[tim][ii]+=belta*rate*sum_x*relation_vec[rel][ii];
        }
        ...
        norm(entity_vec[e2]);
        norm2one(A[tim]);
        //norm(relation_vec[rel],A[rel]);   ////////////////////////// check this
    }
    void train_kb(int e1_a,int e2_a,int rel_a,int tim_a,int e1_b,int e2_b,int rel_b,int tim_b,double &private_res)
    {
	//std::cout << e1_a << " " << e2_a << " " << rel_a << " " << tim_a << endl;
	//std::cout << e1_b << " " << e2_b << " " << rel_b << " " << tim_b << endl;
        double sum1 = calc_sum(e1_a,e2_a,rel_a,tim_a);
        double sum2 = calc_sum(e1_b,e2_b,rel_b,tim_b);
        if (sum1+margin>sum2)
        {
            private_res += margin + sum1 - sum2 ;
            gradient( e1_a, e2_a, rel_a, tim_a, -1) ;
            gradient( e1_b, e2_b, rel_b, tim_b, 1) ;
        }
    }
};
...
{   
    ...
    {
        if(triples[i][3]!=triples[i-1][3])
	    splits.push_back(i);
	
	//std::cout << triples[i][0] << triples[i][1] << triples[i][2] << triples[i][3] << endl ;
    }
